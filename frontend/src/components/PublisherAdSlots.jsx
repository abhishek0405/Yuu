...
function PublisherSlots () {
  ...
  var addGnosisToMetamask = function() {
    ...
    const handleSubmit = async (e) => {
        ...
        const publisherData = await axios.post("http://localhost:5000/publisher/fetchName",{address:walletAddress});
      ...
      console.log("content saved on: ", cid)
        ...
        if(res.data.message === "succesfully posted"){
          ...
          async function fetchAdDetails(){
              var res = await fetch("http://localhost:5000/publisher/renderAd",{ method:"POST", body:JSON.stringify({ hashId:"${cid}"}),headers: {
