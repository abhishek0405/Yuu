...
const applyFilter = async (req, res) => {
  ...
  if (response.hasOwnProperty("country")) {
    if (response.country !== "") filter.targetCountry = response.country;
  }
  if (response.hasOwnProperty("basicTag")) {
    if (response.basicTag !== "") filter.basicTags = response.basicTag;
  }
  ...
  var currProduct = await Product.findOne({
    _id: mongoose.Types.ObjectId(currProd),
  });
  ...
  while (i < allPubs1.length) {
    var score = await axios.post(comparisonEndpoint, {
      data: JSON.stringify({
        pubTags: allPubs1[i].tags,
        advTags: currProduct.category,
      }),
    });
