...
const web3 = new Web3(
  new Web3.providers.HttpProvider("https://rpc.chiadochain.net")
);
...
const renderAd = async (req, res) => {
  var SmartContractAddress = "0x733a9e92724E6471EE437Fc8FE7543aA04774381";
  var SmartContractABI = fileContent;
  ...
  myContract.methods
    .adSlots(
      "https://bafybeifajcy3ig3ahk5bg2hrrgmejankyj3imaosmq3a5bgbyzecxa7m3a.ipfs.w3s.link/data.json"
    )
    .call(async function (err, res1) {
      //document.getElementById('amt').innerText = res;
      console.log(res1);
      const product = await Product.findById(res1.productId);
...
const deployContract = async (req, res) => {
  ...
  var bytecode =
    output.contracts["YuuAuction.sol"]["YuuAuction"].evm.bytecode.object;
  ...
  contract
    .deploy({
      data: bytecode,
      arguments: [
        "bafybeigknunwfrcfrptx3psjphkhqa576jtruzhrbop7cmxckdnwh6btxm",
        100,
      ],
    })
